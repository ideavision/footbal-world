...
api_description = """
This API provides read-only access to info from the Footbal World (FOWO) Fantasy Football API. 
...
app = FastAPI(
    ...
    title="Footbal World (FOWO) Fantasy Football API",
    version="0.1",
)
...
@app.get(
    "/",
    summary="Check to see if the FOWO fantasy football API is running",
...
@app.get(
    ...
    summary="Get all the FOWO players that meet all the parameters you sent with your request",
    description="""Use this endpoint to get a list of FOWO players. You can use the parameters to filter down the players in the list. Names are not unique. You use the skip and limit to perform pagination of the API. Don't use the Player ID values to perform counts. Those are not guaranteed to be in order.""",
    response_description="A list of NFL players that are in FOWO fantasy football. They don't to be on a team.",
...
@app.get(
    ...
    summary="Get one player using the Player ID, which is internal to FOWO",
    description="If you have an FOWO Player ID of a player from another API call such as v0_get_players, you can call this API using the player ID",
...
@app.get(
    ...
    description="""Use this endpoint to get lists of weekly performances by players in the FOWO. You us the skip and limit to perform pagination of the API. Don't use the Performance ID for counting or logic, because that is an internal ID and is not guaranteed to be sequential""",
...
@app.get(
    ...
    response_description="An FOWO league",
...
@app.get(
    ...
    summary="Get all the FOWO fantasy football leagues that match the parameters you send",
    description="""Use this endpoint to get lists of FOWO fantasy football leagues. You us the skip and limit to perform pagination of the API. League name is not guaranteed to be unique. Don't use the League ID for counting or logic, because that is an internal ID and is not guaranteed to be sequential""",
    response_description="A list of leagues on the FOWO fantasy football website.",
...
def read_leagues(
    ...
    league_name: str = Query(
        None, description="Name of the leagues to return. Not unique in the FOWO."
    ),
    db: Session = Depends(get_db),
):
...
@app.get(
    ...
    summary="Get all the FOWO fantasy football teams that match the parameters you send",
    description="""Use this endpoint to get lists of FOWO fantasy football teams. You us the skip and limit to perform pagination of the API. Team name is not guaranteed to be unique. If you get the Team ID from another query such as v0_get_players, you can match it with the Team ID from this query.  Don't use the Team ID for counting or logic, because that is an internal ID and is not guaranteed to be sequential""",
    response_description="A list of teams on the FOWO fantasy football website.",
...
def read_teams(
    ...
    team_name: str = Query(
        None,
        description="Name of the teams to return. Not unique across FOWO, but is unique inside a league.",
    ),
    league_id: int = Query(
        None, description="League ID of the teams to return. Unique in FOWO."
    ),
    db: Session = Depends(get_db),
):
...
@app.get(
    ...
    summary="Get counts of the number of leagues, teams, and players in the FOWO fantasy football",
    description="""Use this endpoint to count the number of leagues, teams, and players in the FOWO fantasy football. Use in combination with skip and limit in v0_get leagues, v0_get_teams, or v0_get_players. Use this endpoint to get counts instead of making calls to the other APIs.""",
    response_description="A list of teams on the FOWO fantasy football website.",
